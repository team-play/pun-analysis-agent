...
- **Test runner: node's built-in `node:test`**, run through `tsx` (already a dev dependency for `pnpm dev`) so tests get the same TypeScript handling as the dev server — `backend/`'s `pnpm test` runs `tsx --test tests/**/*.test.ts`, already wired up: see [`backend/tests/app.test.ts`](../backend/tests/app.test.ts). No new dependency, since a Hono app is just a standard `Request`/`Response` — `app.request()` plus `node:assert` is enough, no framework-specific test adapter needed. This is also why `src/app.ts` exports the `Hono` instance separately from `src/index.ts`'s `serve()` call: tests import the app without booting a real server.
...
- The per-domain deploy workflows ([`deploy-frontend.yml`](../.github/workflows/deploy-frontend.yml), [`deploy-backend.yml`](../.github/workflows/deploy-backend.yml), [`deploy-inference.yml`](../.github/workflows/deploy-inference.yml)) are already scaffolded as path-filtered placeholders, one per domain, per [`local-setup.md`](local-setup.md). Filling in their actual build/deploy steps is a separate, already-flagged follow-up (see the comment at the top of each file) — not something this doc changes. The isolation properties above are what make it safe for those three workflows to deploy independently later without one domain's deploy blocking on another's implementation state.
